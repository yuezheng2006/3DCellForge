...
for car_entry in "${cars[@]}"; do
  ...
  upload_response=$(curl -s -X POST "https://api.tripo3d.ai/v2/openapi/upload" \
  ...
  while [ $attempt -lt $max_attempts ]; do
    ...
    status_response=$(curl -s -X GET "https://api.tripo3d.ai/v2/openapi/task/$task_id" \
      -H "Authorization: Bearer $TRIPO_API_KEY")
    ...
    if [ "$status" = "success" ]; then
      ...
      curl -s -o "$OUTPUT_DIR/${car_name}.glb" "$model_url"
